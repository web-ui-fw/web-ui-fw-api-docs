...
1. Deploy the API docs as shown below. This assumes you have already set up LAMP with wordpress to host local.api.jquerymobile.com and that the docs will overwrite that site.
...
$ wget --mirror -p http://local.api.jquerymobile.com/index/
...
$ git commit -a -m 'Remove shortcut icon'
...
$ git commit -a -m 'Hide branding'
# $ find | grep 'feed$' | xargs rm -rf
# $ git commit -a -m 'Remove feeds'
...
$ git commit -a -m 'Remove pingback, rss, etc. links'
...
$ git commit -a -m 'Make paths relative'
...
$ git checkout jquery-wp-content/
$ git commit -a -m 'Remove forum link'
...
$ git commit -a -m 'Fix issues link'
...
$ git commit -a -m 'Fix references to jQuery API documentation'
$ mv index/index.html .
$ rm -rf index
$
